...
def searchTree(_dir, count):
	...
	for i in range(0, len(_dir)):
		if isinstance(_dir[i], Directory):
			a = searchTree(_dir[i]._dir, 0)
			res.append(a)
			count += a

	return count

curr_size = searchTree(file_system[0]._dir, 0)

free_size = TOTAL_DISK - curr_size
size_to_update = SIZE_REQ_UPDATE - free_size
